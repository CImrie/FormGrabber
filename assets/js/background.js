    ...
    function(request, sender, sendResponse){
        if(request.type == "site")
        {
            chrome.tabs.query({active:true, currentWindow:true},function(tabs){
                var url = (tabs[0].url).replace(/.*?:\/\//g, "");
                url = url.replace(/\/+$/, "");
                var site = new Site({"url": url, added: Date.now(), data: request.data});
    ...
    function(request, sender, sendResponse){
        switch(request.type){
            ...
            case "license-check":
                ...
                sendResponse({type:"license-response", valid: licenseManager.current(), "license": licenseManager.currentLicense});
            break;
        }
    }
);
